...
//   Anthropic側の請求上限が未設定の間の保守的な暫定値として 500/日 にしている。
//   アカウント側の上限が整ったら ARIGATO_CHAT_DAILY_LIMIT で引き上げてよい。
const IP_HOURLY_LIMIT = Number(process.env.ARIGATO_CHAT_IP_HOURLY_LIMIT ?? 60);
const DAILY_LIMIT = Number(process.env.ARIGATO_CHAT_DAILY_LIMIT ?? 500);
