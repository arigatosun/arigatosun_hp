...
const DESIGN_BRANDING: ServiceDetailData = {
  ...
  // \n = PC 専用改行（PC で右に伸びすぎる行を IP 同様に折り返す。SP では無視され流れる）
  description: [
    'モノやコトが溢れる世界において、ブランド自らが声を上げ、\nらしさを振る舞い、情報を正しく伝えていく。',
  ...
  concepts: [
    {
      ...
      // Figma SP (2837:53673) は 5 段落構成。PC では「意匠」「設計」「そして現代…」までを
      // 1 つの流し込み段落にするため \n（= SP 限定改行 spOnlyBr）で連結
      // （PC は改行なしで流す・SP は従来通り段落ごとに改行）。
      body: [
...
const AI_DEV: ServiceDetailData = {
  ...
  description: [
    ...
    '私たちは、止まっている業務を前に進め、\n止まりかけている構想を現実に変えるための基盤をつくる。',
    '人が判断や企画、対話に立ち返るための仕組みを。\n挑戦が、価値を生み出す事業へ変わっていくための土台を。',
...
const IP_CREATIVE: ServiceDetailData = {
  ...
  concepts: [
    ...
    // Figma 改行: 「活かす、」の後で 1 回改行
    {
      ...
      // 配列要素の境目 = PC でも表示される改行（素 <br>）。要素内の \n = SP 限定改行 (.spOnlyBr)。
      // PC では「…担います。」の後（プロデューサーの前）だけ改行し、他は流す。SP は全改行を維持。
      body: [
    ...
    // ⑤ IPの育て方・進め方 ECOSYSTEM PROCESS (Figma 2692:38348 / x=284 y=5100〜)
    {
      ...
      body: [
        // PC は改行なしで流す（\n は SP 限定改行 .spOnlyBr / 多角展開は word-joiner で連結）
        'IPは一度生み出して終わりではなく、\n育てながら次の世代に手渡していくものです。\n発掘・言語化から、世界観構築、メディアミックスによる多⁠角⁠展⁠開、\nそして10年20年と愛され続ける運用\nフェーズまで ―――\n各段階に最適な座組みでプロジェクトを進行します。',
      ],
      bodyTracking: 3.84,
      visual: {
        kind: 'phases',
