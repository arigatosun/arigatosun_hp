...
- **補助手法**: `@include sp / pc` によるレイアウト構造の切り替え（**2 パターン構成**）
...
**2 パターン構成**。スマホとタブレットは同じレイアウトで扱う。

| 名称 | 範囲 | mixin | 用途 |
|---|---|---|---|
| SP（スマホ & タブレット） | **〜1023px** | `@include sp` | 1カラム・縦積み |
| PC（デスクトップ） | **1024px〜** | `@include pc` | フルレイアウト |

> 旧 `@include tab`（768〜1023）は廃止。タブレット域も SP と同じレイアウトを当てる。
> どうしてもタブレット域だけ別レイアウトを当てたい場合は、汎用 mixin `@include min(768px)` + `@include max(1023px)` を組み合わせて表現する。
